...
func (h *ephemeralTerminalUI) Handle(event partybus.Event) error {
	...
	case event.Type == xeolEvent.EolScanningFinished:
		...
		if err := handleEolScanningFinished(event, h.reportOutput); err != nil {
			log.Errorf("unable to show %s event: %+v", event.Type, err)
		}
	...
	case event.Type == xeolEvent.NonRootCommandFinished:
		h.closeScreen(false)
