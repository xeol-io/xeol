...
var completionCmd = &cobra.Command{
	...
	Short: "Generate a shell completion for xeol (listing local docker images)",
	Long: `To load completions (docker image list):

Bash:

$ source <(xeol completion bash)

# To load completions for each session, execute once:
Linux:
  $ xeol completion bash > /etc/bash_completion.d/xeol
MacOS:
  $ xeol completion bash > /usr/local/etc/bash_completion.d/xeol
...
# To load completions for each session, execute once:
$ xeol completion zsh > "${fpath[1]}/_xeol"

# You will need to start a new shell for this setup to take effect.

Fish:

$ xeol completion fish | source

# To load completions for each session, execute once:
$ xeol completion fish > ~/.config/fish/completions/xeol.fish
`,
...
func dockerImageValidArgsFunction(_ *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
