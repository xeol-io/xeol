...
test_extract_json_value() {
  fixture=./test-fixtures/github-api-xeol-v0.2.2-release.json
  content=$(cat ${fixture})

  actual=$(extract_json_value "${content}" "tag_name")
  assertEquals "v0.2.2" "${actual}" "unable to find tag_name"
...
test_github_release_tag() {
  ...
  assertEquals "v0.2.2" "${actual}" "unable to find release tag"
}
...
test_download_github_release_checksums() {
  ...
  assertFilesEqual \
    "./test-fixtures/xeol_0.2.2_checksums.txt" \
    "${actual_filepath}" \
