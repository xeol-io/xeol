...
func getXeolCommand(tb testing.TB, args ...string) *exec.Cmd {
	tb.Helper()

	return exec.Command(
		getXeolSnapshotLocation(tb, runtime.GOOS),
		append(
			[]string{"-c", "../xeol-test-config.yaml"},
			args...,
		)...,
	)
}

func getXeolSnapshotLocation(tb testing.TB, goOS string) string {
	if os.Getenv("XEOL_BINARY_LOCATION") != "" {
		// XEOL_BINARY_LOCATION is the absolute path to the snapshot binary
		return os.Getenv("XEOL_BINARY_LOCATION")
	}
	...
	switch goOS {
	case "darwin", "linux":
		return path.Join(repoRoot(tb), fmt.Sprintf("snapshot/%s-build_%s_%s/xeol", goOS, goOS, archPath))
	default:
...
func runXeol(tb testing.TB, env map[string]string, args ...string) (*exec.Cmd, string, string) {
	tb.Helper()

	cmd := getXeolCommand(tb, args...)
	if env == nil {
		env = make(map[string]string)
	}

	// we should not have tests reaching out for app update checks
	env["XEOL_CHECK_FOR_APP_UPDATE"] = "false"
